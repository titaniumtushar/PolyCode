...
const ContestPage: React.FC = () => {
    ...
    const [contestToRegister, setContestToRegister] = useState<any | null>(
        null
    ); // Track contest to register for
            ...
            const response = await fetch(
                `${API_URL}/api/user/contest/register`,
                {
                    ...
                    body: JSON.stringify({
                        ...
                        contest_id: selectedContest._id, // Use selectedContest to get the correct contest id
                    }),
                }
            );
            ...
            {!selectedContest ? (
                        ...
                        {contests.map((contest, index) => (
                            ...
                            >
                                ...
                                <p className="text-sm text-gray-400">
                                    Start:{" "}
                                    {new Date(
                                        contest.start_time * 1000
                                    ).toLocaleString()}
                                </p>
                                <p className="text-sm text-gray-400">
                                    End:{" "}
                                    {new Date(
                                        contest.end_time * 1000
                                    ).toLocaleString()}
                                </p>
                                <p className="text-sm text-gray-300">
                                    Prize Distribution:{" "}
                                    {contest.meta.prize_distribution.join(", ")}
                                </p>
                            </div>
                        ))}
                    </div>
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-400">
                        Start:{" "}
                        {new Date(
                            selectedContest.start_time * 1000
                        ).toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-400">
                        End:{" "}
                    ...
                    <p className="text-sm text-gray-300">
                        Prize Distribution:{" "}
                        {selectedContest.meta.prize_distribution.join(", ")}
                    </p>
                    ...
                    selectedContest.end_time > new Date().valueOf() / 1000 ? (
                        <div className="mt-4 flex justify-start gap-4">
                            ...
                            <button
                                className="py-2 px-4 bg-blue-600 text-white rounded hover:bg-blue-700"
                                onClick={() =>
                                    handleRegisterContest(selectedContest)
                                }
                            >
                                Register
                            </button>
                        ...
                        <CodeBlock
                            input="This contest is not live"
                            status="lij"
                        />
                    )}
                </div>
